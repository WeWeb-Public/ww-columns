<template>
    <div class="ww-columns-popup-layout ww-scroll-bar">
        <div class="columns-count-container">
            <wwManagerInput
                class="columns-count"
                :label="wwLang.getText('columnsCount')"
                outline="true"
                v-model="columnsCount"
            ></wwManagerInput>
            <wwManagerButton class="button" color="blue" @click="applyColumnCount()">{{
                wwLang.getText('apply')
            }}</wwManagerButton>
        </div>
        ...
        <div class="preview-container">
            <div class="preview" :style="wrap()">
                <div
                    class="column"
                    v-for="(column, index) in screenCols"
                    :key="index"
        ...
        <div class="config-tabs" v-if="!config[screen].ignore">
            <div class="tab" :class="{ active: mode == 'size' }" @click="mode = 'size'">
                {{ wwLang.getText('sizes') }}
            </div>
            <div class="tab" :class="{ active: mode == 'border' }" @click="mode = 'border'">
                {{ wwLang.getText('borders') }}
            </div>
        ...
        <div class="config-container" v-if="!config[screen].ignore">
            ...
            <div class="config" v-if="mode == 'size'">
                <div class="columns-count-container">
                    <wwManagerButton class="button" color="blue" @click="resizeColumns(false)">{{
                        wwLang.getText('distribute')
                    }}</wwManagerButton>
                    <wwManagerButton class="button" color="blue" @click="resizeColumns(true)">{{
                        wwLang.getText('distributeWithSpaces')
                    }}</wwManagerButton>
                </div>
                <div class="columns-count-container">
                    <wwManagerInput
                        color="green"
                        :label="wwLang.getText('height')"
                        v-model="config[screen].height"
                    ></wwManagerInput
                    >&nbsp;
                    <wwManagerSelect
                        class="select"
                        v-model="config[screen].unit"
                        :options="unitOptions"
                        @change="setUnit(config[screen], $event)"
                    ></wwManagerSelect
                    >&nbsp;&nbsp;&nbsp;
                    <wwManagerSelect
                        class="select"
                        v-model="config[screen].nowrap"
                        :options="wrapOptions"
                        @change="setWrap(config[screen], $event)"
                    ></wwManagerSelect>
                </div>
                <div class="columns">
                    <div class="column" v-for="(column, index) in screenCols" :key="index" :class="getColor(index)">
                        ...
                        <div class="content-size">
                            <wwManagerInput
                                ...
                                :label="wwLang.getText('offset') + ' - %'"
                                v-model="column.offset"
                            ></wwManagerInput>
                            <wwManagerInput
                                :color="getColor(index)"
                                :label="wwLang.getText('width') + ' - %'"
                ...
                <div class="columns">
                    <div class="column" v-for="(column, index) in screenCols" :key="index" :class="getColor(index)">
                        <div class="copy-paste-container">
                            <div
                                class="copy-paste"
                                v-if="copiedColumnConfig.border"
                                @click="pasteConfig('border', index)"
                            >
                                <div class="wwi wwi-paste"></div>
                            </div>
                        ...
                        <div class="content-border">
                            <div class="border" v-for="i in 4" :key="i">
                                <wwManagerInput
                                    :color="getColor(index)"
                                    :label="wwLang.getText(borderNames[i - 1])"
                                    v-model="column.borders[i - 1].width"
                                    @change="setBorderStyle(column, i - 1, $event)"
                                ></wwManagerInput>
                                <wwManagerSelect
                                    class="select"
                                    v-model="column.borders[i - 1].style"
                                    :options="borderStyleOptions"
                                    @change="setBorderWidth(column, i - 1, $event)"
                                ></wwManagerSelect>
...
export default {
    ...
    methods: {
        init() {
            ...
            this.config = this.options.data.content.config;
            // this.correctConfigs();
            ...
            for (let screen of this.screens) {
                ...
                for (let i = 0; i < this.config.count; i++) {
                    ...
                    if (confCols.length > i) {
                        ...
                        confCols[i].borders =
                            confCols[i].borders && confCols[i].borders.length == 4
                                ? confCols[i].borders
                                : JSON.parse(JSON.stringify(this.defaultBorders));
        ...
        copyConfig(type, index) {
            switch (type) {
                case 'size':
                    ...
                    break;
                case 'border':
                    this.copiedColumnConfig.border = JSON.parse(
                        JSON.stringify(this.config[this.screen].cols[index].borders)
                    );
                    //this.copiedColumnConfig.radius = JSON.parse(JSON.stringify(this.config[this.screen].cols[index].radius));
                    break;
                case 'shadow':
                    this.copiedColumnConfig.shadow = JSON.parse(
                        JSON.stringify(this.config[this.screen].cols[index].shadow)
                    );
                    break;
                default:
                    break;
        ...
        pasteConfig(type, index) {
            switch (type) {
                ...
                case 'border':
                    Vue.set(
                        this.config[this.screen].cols[index],
                        'borders',
                        JSON.parse(JSON.stringify(this.copiedColumnConfig.border))
                    );
                    break;
                case 'shadow':
                    Vue.set(
                        this.config[this.screen].cols[index],
                        'shadow',
                        JSON.parse(JSON.stringify(this.copiedColumnConfig.shadow))
                    );
                    break;
                default:
                    break;
